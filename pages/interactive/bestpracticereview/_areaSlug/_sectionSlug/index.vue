<template>
  <section class="container-fluid">
    ...
    <div class="row" style="cursor: pointer;" @click="$router.push({name: 'interactive-bestpracticereview-areaSlug', params: {areaSlug: area.data.slug}})">
    ...
    <div class="row" style="cursor: pointer;" v-for="(item, index) in section.data.items" @click="$router.push({name: 'interactive-bestpracticereview-areaSlug-sectionSlug-itemId', params: {areaSlug: area.data.slug, sectionSlug: section.data.slug, itemId: index + 1}})">
      <div class="col-6 py-2" style="overflow: hidden;" :style="{borderBottom: `1px solid ${area.data.color_dark}`}">
        <span style="font-size: 200%;">{{ item.left.en }}</span>
      </div>

      <div class="col-6 py-2" style="overflow: hidden;" :style="{borderBottom: `1px solid ${area.data.color_dark}`}">
        <span style="font-size: 200%;">{{ item.left.es }}</span>
      </div>
    </div>
...
export default {
  ...
  methods: {
    ...
    previous () {
      const {bestPractices, area, section} = this
      ...
      if (newSectionIndex > -1) {
        this.$router.push({
          name: 'interactive-bestpracticereview-areaSlug-sectionSlug',
          params: {
    ...
    next () {
      ...
      if (newSectionIndex < area.data.sections.length) {
        this.$router.push({
          name: 'interactive-bestpracticereview-areaSlug-sectionSlug',
          params: {
            areaSlug: area.data.slug,
